...
(deftest tictactoe-test
  ...
  (testing "TicTacToe should consider the game finished when there are no empty places in the board"
    (let [ttt-instance (TicTacToe.)]
      (is (is-finished? ttt-instance [["O" "X" "O"] ["X" "O" "X"] ["O" "X" "O"]]))))

  (testing "TicTacToe should consider the game finished when there is a row filled with the same marker"
    (let [ttt-instance (TicTacToe.)]
      (let [game-state [["X" "X" "X"] ["E" "E" "E"] ["E" "E" "E"]]
            result (is-finished? ttt-instance game-state)
            winner (get-winner ttt-instance game-state)]
        (is result)
        (is (= winner 0)))
      (let [game-state [["E" "E" "E"] ["O" "O" "O"] ["E" "E" "E"]]
            result (is-finished? ttt-instance game-state)
            winner (get-winner ttt-instance game-state)]
        (is result)
        (is (= winner 1)))
      (let [game-state [["E" "E" "E"] ["E" "E" "E"] ["X" "X" "X"]]
            result (is-finished? ttt-instance game-state)
            winner (get-winner ttt-instance game-state)]
        (is result)
        (is (= winner 0)))))
  ...
  (testing "TicTacToe should consider the game finished when there is a column filled with the same marker"
    (let [ttt-instance (TicTacToe.)]
      (let [game-state [["X" "E" "E"] ["X" "E" "E"] ["X" "E" "E"]]
            result (is-finished? ttt-instance game-state)
            winner (get-winner ttt-instance game-state)]
        (is result)
        (is (= winner 0)))
      (let [game-state [["E" "O" "E"] ["E" "O" "E"] ["X" "O" "X"]]
            result (is-finished? ttt-instance game-state)
            winner (get-winner ttt-instance game-state)]
        (is result)
        (is (= winner 1)))
      (let [game-state [["E" "E" "X"] ["E" "E" "X"] ["E" "E" "X"]]
            result (is-finished? ttt-instance game-state)
            winner (get-winner ttt-instance game-state)]
        (is result)
        (is (= winner 0)))))
  ...
  (testing "TicTacToe should consider the game finished when there is a diagonal filled with the same marker"
    (let [ttt-instance (TicTacToe.)]
      (let [game-state [["X" "E" "E"] ["E" "X" "E"] ["E" "E" "X"]]
            result (is-finished? ttt-instance game-state)
            winner (get-winner ttt-instance game-state)]
        (is result)
        (is (= winner 0)))
      (let [game-state [["E" "E" "O"] ["E" "O" "E"] ["O" "E" "E"]]
            result (is-finished? ttt-instance game-state)
            winner (get-winner ttt-instance game-state)]
        (is result)
        (is (= winner 1)))))
